...
const ROAD_TYPES = [
  ...
  { value: 'PLANE', label: '✈️ Air Route', icon: '✈️' }
];

// Road validation schema
const roadValidation = {
  initialPlace: [rules.required('Initial place'), rules.minLength('Initial place', 2)],
  roadType: [rules.required('Road type')]
};
...
const RoadsManagementPage = () => {
  ...
  const validateFormData = (): boolean => {
    const errors = validateForm(formData, roadValidation);
  ...
  const handleCreate = async () => {
    ...
    try {
      setActionLoading(-1);
      await AdminRoadService.createRoad(token, { ...formData, tourismPlaceId: selectedTourismId });
  ...
  const handleUpdate = async () => {
    if (!token || !editingRoad || !selectedTourismId) return;
    setFormError('');
    if (!validateFormData()) return;
  ...
  const getRoadTypeIcon = (type: string) => {
    const found = ROAD_TYPES.find(rt => rt.value === type);
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">🛣️ Roads Management</h1>
              <p className="text-gray-600">Manage travel routes and pathways for tourism places</p>
            </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="flex-1 max-w-md">
              ...
              <select value={selectedTourismId || ''} onChange={(e) => setSelectedTourismId(e.target.value ? parseInt(e.target.value) : null)}
                className="w-full bg-white border border-gray-300 text-gray-900 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <input type="text" placeholder="Search roads..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full pl-10 pr-4 py-2 bg-white border border-gray-300 text-gray-900 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
          {!selectedTourismId ? (
            <div className="p-8 text-center text-gray-500">
              ...
              <p className="text-lg">Please select a tourism place to view its roads</p>
            </div>
          ) : loading ? (
            <div className="p-8 text-center">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 mx-auto"></div>
              <p className="mt-4 text-gray-500">Loading roads...</p>
            </div>
          ) : error ? (
            <div className="p-8 text-center">
              <Alert type="error" message={error} />
              <button onClick={() => selectedTourismId && loadRoads(selectedTourismId)} className="mt-4 bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700">Retry</button>
            </div>
          ) : filteredRoads.length === 0 ? (
            <div className="p-8 text-center text-gray-500">
              <span className="text-5xl mb-4 block">🛣️</span>
              <p className="text-lg mb-4">No roads found for {getSelectedTourismName()}</p>
              <button onClick={() => { resetForm(); setShowModal(true); }} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">Add First Road</button>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Road Type</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Initial Place</th>
                    ...
                    <tr key={road.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <span className="text-2xl mr-3">{getRoadTypeIcon(road.roadType)}</span>
                          <div>
                            <div className="text-sm font-medium text-gray-900">{road.roadType}</div>
                            <div className="text-xs text-gray-500">ID: {road.id}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-wrap gap-1">
                          {road.distanceByCar && <span className="px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded">🚗 {road.distanceByCar}km</span>}
                          {road.distanceByFoot && <span className="px-2 py-1 text-xs bg-green-100 text-green-700 rounded">🚶 {road.distanceByFoot}km</span>}
                          {road.distanceByHorse && <span className="px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded">🐎 {road.distanceByHorse}km</span>}
                          {road.distanceByPlane && <span className="px-2 py-1 text-xs bg-orange-100 text-orange-700 rounded">✈️ {road.distanceByPlane}km</span>}
                          {road.totalDistance && <span className="px-2 py-1 text-xs bg-gray-100 text-gray-700 rounded">Total: {road.totalDistance}km</span>}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <p className="text-sm text-gray-600 line-clamp-2 max-w-xs">{road.description || 'No description'}</p>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex space-x-3">
                          <button onClick={() => openEditModal(road)} className="text-blue-600 hover:text-blue-800 font-medium">Edit</button>
                          <button onClick={() => handleDelete(road.id)} disabled={actionLoading === road.id}
                            className="text-red-600 hover:text-red-800 font-medium disabled:opacity-50">
                            {actionLoading === road.id ? 'Deleting...' : 'Delete'}
                          </button>
                        </div>
                      </td>
                    </tr>
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto border border-gray-200">
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <select name="roadType" value={formData.roadType} onChange={handleInputChange}
                  ...
                  {formErrors.roadType && <p className="mt-1 text-sm text-red-600">{formErrors.roadType}</p>}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">🚗 Distance by Car (km)</label>
                    <input type="number" name="distanceByCar" value={formData.distanceByCar || ''} onChange={handleInputChange}
                      placeholder="0" min={0} step={0.1}
                      className="w-full bg-white border border-gray-300 text-gray-900 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">🚶 Distance by Foot (km)</label>
                    <input type="number" name="distanceByFoot" value={formData.distanceByFoot || ''} onChange={handleInputChange}
                      placeholder="0" min={0} step={0.1}
                      className="w-full bg-white border border-gray-300 text-gray-900 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">🐎 Distance by Horse (km)</label>
                    <input type="number" name="distanceByHorse" value={formData.distanceByHorse || ''} onChange={handleInputChange}
                      placeholder="0" min={0} step={0.1}
                      className="w-full bg-white border border-gray-300 text-gray-900 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">✈️ Distance by Plane (km)</label>
                    <input type="number" name="distanceByPlane" value={formData.distanceByPlane || ''} onChange={handleInputChange}
                      placeholder="0" min={0} step={0.1}
                      className="w-full bg-white border border-gray-300 text-gray-900 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input type="number" name="totalDistance" value={formData.totalDistance || ''} onChange={handleInputChange}
                    placeholder="0" min={0} step={0.1}
                    className="w-full bg-white border border-gray-300 text-gray-900 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500" />
                </div>
