...
const HotelsManagementPage = () => {
  ...
  const openEditModal = (hotel: Hotel) => {
    ...
    setFormData({
      ...
      tourismPlaceId: hotel.tourismPlaceId || hotel.tourismId || 0, starRating: hotel.starRating || hotel.stars || 3,
    ...
    <div className="min-h-screen bg-gray-200 admin-page">
      ...
      <div className="mb-8 bg-gradient-to-r from-slate-900 via-blue-900 to-slate-900 p-6 rounded-xl shadow-xl">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-black text-white mb-2">🏨 Hotel Management</h1>
            <p className="text-blue-200 font-semibold">Manage hotels, assign owners, and control booking availability</p>
          </div>
          <button onClick={() => { resetForm(); setShowModal(true); }}
            className="bg-emerald-500 text-white px-4 py-2 rounded-lg hover:bg-emerald-600 flex items-center gap-2 transition-colors font-black shadow-lg">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {/* Search */}
      <div className="bg-green-100 rounded-xl shadow-xl p-6 mb-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" placeholder="Search hotels by name, description, or contact..."
              ...
              className="w-full pl-10 pr-4 py-2 border-2 border-green-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 font-bold bg-white shadow-sm" />
          </div>
          <div className="text-sm text-gray-900 bg-white px-4 py-2 rounded-lg font-bold shadow-md">
            Total Hotels: <span className="font-black text-green-700">{totalElements}</span>
          </div>
        </div>
      </div>

      {/* Hotels Grid */}
      <div className="bg-green-100 rounded-xl shadow-xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center bg-white">
            ...
            <button onClick={loadHotels} className="mt-4 bg-green-200 text-green-800 px-4 py-2 rounded-md hover:bg-green-300 font-black shadow-md">Retry</button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
            {filteredHotels.map((hotel) => (
              <div key={hotel.id} className="rounded-xl overflow-hidden shadow-2xl hover:shadow-[0_20px_50px_rgba(0,0,0,0.3)] transition-all duration-300 group bg-white hover:-translate-y-1">
                ...
                <div className="p-4 bg-green-50 shadow-inner">
                  {/* Hotel Name */}
                  <h3 className="text-xl font-black text-gray-900 group-hover:text-green-700 transition-colors">{hotel.name}</h3>
                  
                  {/* Phone */}
                  <p className="text-sm font-black text-green-700 mt-2 flex items-center gap-2">
                  ...
                  <div className="mt-4 flex flex-col gap-2">
                    <button onClick={() => router.push(`/admin/hotels/${hotel.id}`)}
                      className="w-full py-2 bg-green-200 text-green-800 hover:bg-green-300 rounded-lg text-sm font-black transition-all shadow-md hover:shadow-lg hover:scale-105">
                      🔧 Manage
                    </button>
